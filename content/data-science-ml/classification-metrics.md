...
3つの独立した実践例では、Accuracy だけではだめに関連して、目的を絞って小さく実装し、結果を確認しながら改善する流れが共通していた。環境やデータで結果は変わるため条件付きで支持する。
...
### 1. ロジスティック回帰、決定木、k近傍法の分類器を同じデータで比較した
